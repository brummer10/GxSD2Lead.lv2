...
#define GTK_SD2LEAD_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_SD2LEAD_KNOB, GtkSD2LeadKnobPrivate))

G_DEFINE_TYPE(GtkSD2LeadKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkSD2LeadKnobPrivate GtkSD2LeadKnobPrivate;

struct _GtkSD2LeadKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkSD2LeadKnobPrivate *priv = GTK_SD2LEAD_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkSD2LeadKnob *knob = GTK_SD2LEAD_KNOB(widget);
...
{
    g_assert(GTK_IS_SD2LEAD_KNOB(widget));
    GtkSD2LeadKnob *knob = GTK_SD2LEAD_KNOB(widget);
    GtkSD2LeadKnobPrivate *priv = GTK_SD2LEAD_KNOB_GET_PRIVATE(knob);
...
{
    GtkSD2LeadKnob *knob = GTK_SD2LEAD_KNOB(object);
...
/****************************************************************
 ** init the GtkSD2LeadKnobClass
 */

static void gtk_knob_class_init (GtkSD2LeadKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkSD2LeadKnobPrivate));
}
...
static void gtk_knob_init (GtkSD2LeadKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_SD2LEAD_KNOB, NULL ));
