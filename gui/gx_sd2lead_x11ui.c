...
static void resize_event(gx_sd2leadUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_sd2leadUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_sd2leadUI* ui = (gx_sd2leadUI*)malloc(sizeof(gx_sd2leadUI));
	...
	ui->controls[0] = { 1.0, 1.0, 0.0, 1.0, 1.0, 50, 30, 61, 61, false,"POWER", BSWITCH, BYPASS};
	ui->controls[1] = { 0.5, 0.5, 0.0, 1.0, 0.01, 110, 30, 61, 61, false,"DRIVE", KNOB, DRIVE};
	ui->controls[2] = { -16.0, -16.0, -20.0, 4.0, 0.5, 180, 30, 61, 61, false,"LEVEL", KNOB, LEVEL};
	ui->controls[3] = { 400.0, 400.0, 100.0, 1000.0, 10.3, 250, 30, 61, 61, false,"TONE", KNOB, TONE};
...
static void cleanup(LV2UI_Handle handle) {
	gx_sd2leadUI* ui = (gx_sd2leadUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_sd2leadUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_sd2leadUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_sd2leadUI *ui, gx_controller* controller) {
...
static void _expose(gx_sd2leadUI *ui) {
	static const char* plug_name = "GxSD2Lead" ;
...
static void controller_expose(gx_sd2leadUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_sd2leadUI *ui, int controller) {
...
static void check_value_changed(gx_sd2leadUI *ui, int i, float* value) {
...
static void check_is_active(gx_sd2leadUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_sd2leadUI *ui) {
...
static bool get_active_ctl_num(gx_sd2leadUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_sd2leadUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_sd2leadUI *ui, int direction) {
	float value;
	int num;
...
static void switch_event(gx_sd2leadUI *ui, int i) {
...
static void button1_event(gx_sd2leadUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_sd2leadUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_sd2leadUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_sd2leadUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_sd2leadUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_sd2leadUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
void get_last_active_controller(gx_sd2leadUI *ui, bool set) {
...
static void event_handler(gx_sd2leadUI *ui) {
	XEvent xev;
